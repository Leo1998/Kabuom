...
public abstract class LightView extends View {
    ...
    public LightView(int width, int height, ViewManager viewManager) {
    ...
    @Override
    public final void renderScene(float deltaTime, Batch batch) {
        batch.end();
        ...
        batch.begin(getSceneCamera());
        GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        renderNonOccluders(batch, this.getSceneCamera());
        batch.end();

        batch.begin(getSceneCamera(), lightingShader);
    ...
    private void updateShadowMaps(Batch batch) {
        for (int i = 0; i < lights.size(); i++) {
            ...
            batch.begin(lightCamera, shadowMapRenderShader);
